...
class PyQComposedBlock(pyq.QuantumCircuit):
    def __init__(
        ...
        """
        Merge operations that are adjacent and have identical qubit_support. It results in fewer call of apply_operator
        """
    ...
    def  forward(self, state: Tensor, values: dict[str, Tensor] | None = None) -> Tensor:
        ...
        return reduce(lambda y,x: apply_operator(state = y, operator = x[0], qubits = x[1])  ,zip( self.merged_unitary(values,batch_size),self.merged_qubits_support ),state)
