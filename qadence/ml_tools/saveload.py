...
def write_checkpoint(
    ...
    model_checkpoint_name: str = (
        f"model_{type(model).__name__}_ckpt_" + f"{iteration_substring}" + f"_device_{device}" + ".pt"
    )
    opt_checkpoint_name: str = (
        f"opt_{type(optimizer).__name__}_ckpt_" + f"{iteration_substring}" + f"_device_{device}" + ".pt"
    )
    try:
        d = (
