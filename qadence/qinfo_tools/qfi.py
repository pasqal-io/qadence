...
from qadence import Overlap, OverlapMethod
from qadence.circuit import QuantumCircuit
from qadence.backend import BackendName
from qadence.measurements import Measurements
from qadence import DiffMode
from qadence.blocks import primitive_blocks, parameters
...
def _overlap_with_inputs(
    ...
    """Gives an OverlapModel consisting of the overlap of the circuit with itself. The vparamaters
...
def get_quantum_fisher(
    ...
    """Function to calculate the exact Quantum Fisher Information (QFI) matrix of the quantum state generated
    by the quantum circuit. The QFI matrix is calculated as the Hessian of the fidelity of the quantum state
    ...
    # Which means if we differentiate wrt vparams we are differentiating only wrt the parameters in the bra
    # and not in the ket
...
def get_quantum_fisher_spsa(
    ...
    """Function to calculate the Quantum Fisher Information (QFI) matrix of the quantum state generated
    by the quantum circuit. The QFI matrix is calculated as the Hessian of the fidelity of the quantum state
    ...
    if k == 0:
        ...
        qfi_mat_estimator = (1 / (k + 1)) * (k * previous_qfi_estimator + qfi_mat)
...
#     # Get the positive-semidefinite version of the matrix for the update rule in QNG
#     qfi_mat_positive_sd = symsqrt(torch.matmul(qfi_mat, qfi_mat))

#     # Regularize to ensure invertibility
#     qfi_mat_positive_sd = qfi_mat + beta * torch.eye(num_vparams)
