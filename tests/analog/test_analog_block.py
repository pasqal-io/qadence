...
from qadence.operations import AnalogInteraction, AnalogRot, AnalogRX, X
...
def test_analog_block() -> None:
    b: AnalogBlock
    b = AnalogInteraction(duration=3)
    assert repr(b) == "InteractionBlock(t=3.0, support=(1, 2))"

    c1 = chain(
        AnalogRot(qubit_support=(0, 1), duration=2000, omega=1, delta=0, phase=0),
        AnalogRot(qubit_support=(0, 1), duration=2000, omega=1, delta=0, phase=0),
    )
    ...
    c2 = kron(
        AnalogRX((0, 1), PI),
        AnalogInteraction(duration=1000),
    )
    ...
    with pytest.raises(ValueError, match="Blocks with global support cannot be kron'ed."):
        kron(AnalogRX((0, 1), PI), AnalogInteraction(duration=1000))
